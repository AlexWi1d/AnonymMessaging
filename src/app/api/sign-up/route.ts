...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingUserVeriffiedByUsername = await UserModel.findOne({
      username,
      isVarified: true,
    });

    if (existingUserVeriffiedByUsername) {
    ...
    return Response.json(
      {
        success: true,
        message: "User registered successfully. Please check your email for varification"
      },
      { status: 201 }
    );
  } catch (error) {
